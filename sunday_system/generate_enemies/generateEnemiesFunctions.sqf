	...
	if (count _bunkerPos > 0) then {
		...
		switch (_bunkerType) do {
			case "Land_BagBunker_Large_F": {				
				_numBunkerGuards = [round (3 * aiMultiplier), round (5 * aiMultiplier)] call BIS_fnc_randomInt;
				_leader = nil;
				_leaderChosen = 0;
			...
			case "Land_BagBunker_Tower_F": {					
				_numBunkerGuards = [round (2 * aiMultiplier), round (4 * aiMultiplier)] call BIS_fnc_randomInt;
				_leader = nil;
				_leaderChosen = 0;
				for "_n" from 1 to _numBunkerGuards do {
					_dir = random 360;					
