...
switch (_hvtStyle) do {
	case "INSIDE": {				
			...
			_sectorIndex = [_hvtPos] call dco_closestSectorIndex;
			diag_log format ["_sectorIndex = %1", _sectorIndex];
	};
	case "OUTSIDE": {										
		_hvtPos = [AO_flatPositions] call sun_selectRemove;		
...
_hvtChar allowFleeing 1;	

// Marker
_markerName = format["hvtMkr%1", floor(random 10000)];
_hvtMarker = createMarker [_markerName, _hvtPos];
_hvtMarker setMarkerShape "ICON";
_hvtMarker setMarkerType  "Empty";
